...
import AvailableJobTable from './AvailableJobTable';
...
const JobPortal = () => {
    ...
    const handleButton = (e) => {
        console.log(e);
        if (e.target.innerText == 'APPLIED JOBS') {
        ...
        <Box sx={{ width: '100%', height: '100vh', paddingX: '46px', display: 'flex', flexDirection: 'column', overflow: 'auto', position: 'relative', zIndex: '10' }}>
            {
                ...
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '12px' }}>
                    <Button color='warning' variant={selected1[0]} sx={{ marginTop: '104px' }} onClick={handleButton}>Available Jobs</Button>
                    <Button color='warning' variant={selected2[0]} sx={{ marginTop: '104px' }} onClick={handleButton}>Applied Jobs</Button>
                </Box> : null
            }
