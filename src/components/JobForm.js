    ...
    return (
        <Box sx={{ paddingX: '46px', height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ height: '90px', display: 'flex', alignItems: 'center', textTransform: 'capitalize' }}>
                <Typography variant='h5' fontWeight='bold' color={purple[600]}>Add Job</Typography>
            </Box>
            <Box sx={{ marginBottom: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <Box sx={{ display: 'flex', flexDirection: 'row', gap: '12px' }}>
                    <CustomTextField sx={{ flexGrow: 1 }} label='Job name' color="secondary" onChange={(e) => setCname(e.target.value)} />
                    <CustomTextField sx={{ flexGrow: 1 }} label='Employer name' color="secondary" onChange={(e) => setEname(e.target.value)} />
                </Box>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <CustomTextField fullWidth label='Job URL' color="secondary" onChange={(e) => setURL(e.target.value)} />
